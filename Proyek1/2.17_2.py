...
for t in range(tmax):
    ...
    for i in range(predator_swarm):
        error = (1 - predator[i][0]) ** 2 + 1 * (predator[i][1] - predator[i][0] ** 2) ** 2
        if predator_best_value[i] > error:
            predator_best_value[i] = error
            predator_best_value[i] = predator[i]
...
#Predators
    for i in range(predator_swarm):
        ...
        temp_y = position[0][0] - predator[i][0]
        length = np.sqrt(np.power(temp_x, 2.0) * np.power(temp_x, 2.0))
        # temp_length = length
        # if length < 0:
        #     temp_length = temp_length * -1

        for k in range(num_swarm):
            ttemp_x = position[k][1] - predator[i][1]
            ttemp_y = position[k][0] - predator[i][0]
            tlength = np.sqrt(np.power(ttemp_x, 2.0) * np.power(ttemp_x, 2.0))
        ...
        predator_velocity[i][0] += c1 * np.random.rand() * (predator_best_position[i][0] - temp_y) \
                   + c2 * np.random.rand()
...
#2D
    ...
    #Predators
    for i in range(predator_swarm):
        plt.plot(predator[i][0], predator[i][1], 'bo')

# #3D
#     for i in range(num_swarm):
#         ax.scatter(position[i][0], position[i][1], 0 ,color='red')
#     for i in range(predator_swarm):
#         ax.scatter(predator[i][0], predator[i][1], 0 ,color='blue')
