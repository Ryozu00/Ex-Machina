    ...
    for k in range(predator_swarm):
        for i in range(num_swarm):
            ...
            velocity[i] += c1 * np.random.rand() * (personal_best_position[i] - position[i]) \
                           + c2 * np.random.rand() * (predator[k] - position[i])
            position[i] -= velocity[i]

    for i in range(predator_swarm):
        for k in range(num_swarm):
            # update predator velocity
            predator_velocity[i] += c1 * np.random.rand() * (predator_best_position[i] - predator[i]) \
                           + c2 * np.random.rand() * (position[k] - predator[i])
