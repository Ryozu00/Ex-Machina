...
num_swarm = 100
num_func_params = 2
position = 1 + 4 * np.random.rand(num_swarm, num_func_params)
...
predator_swarm = 100
predator_params = 2
predator = 1 + 4 * np.random.rand(predator_swarm, predator_params)
...
for i in range(num_swarm):
    temp = temp + i * np.cos((i + 1) * position[i][0] + 1)
for i in range(num_swarm):
    temp2 = temp + i * np.cos((i + 1) * position[i][1] + 1)
for i in range(num_swarm):
    personal_best_value[i] = temp * temp2

temp = 0
temp2 = 0
for i in range(predator_swarm):
    temp = temp + i * np.cos((i + 1) * predator[i][0] + 1)
for i in range(predator_swarm):
    temp2 = temp + i * np.cos((i + 1) * predator[i][1] + 1)
for i in range(predator_swarm):
    predator_best_value[i] = temp * temp2
...
#Foods
    ...
    for i in range(num_swarm):
        ...
        if (position[i][0] - velocity[i][0] >= 5) or (position[i][0] - velocity[i][0] <= 1):
            velocity[i][0] *= -1
            
        if (position[i][1] - velocity[i][1] >= 5) or (position[i][1] - velocity[i][1] <= 1):
            velocity[i][1] *= -1
...
#Predators
    for i in range(predator_swarm):
        ...
        if (predator[i][0] + predator_velocity[i][0] >= 5) or (predator[i][0] + predator_velocity[i][0] <= 1):
            predator_velocity[i][0] *= -1
        if (predator[i][1] + predator_velocity[i][1] >= 5) or (predator[i][1] + predator_velocity[i][1] <= 1):
