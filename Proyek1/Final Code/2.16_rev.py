...
num_swarm = 50
num_func_params = 2
position = -5 + 10 * np.random.rand(num_swarm, num_func_params)
...
predator_swarm = 50
predator_params = 2
predator = -5 + 10 * np.random.rand(predator_swarm, predator_params)
...
for i in range (num_swarm):
    for j in range(num_swarm):
        temp = temp + 1/(5*(i+2)+j+3+(position[i][0] - (16*j))**6+(position[i][1]-(16*i))**6)
for i in range(num_swarm):
    personal_best_value[i] = 1 / (0.002 + temp)

temp = 0
for i in range (predator_swarm):
    for j in range(predator_swarm):
        temp = temp + 1/(5*(i+2)+j+3+(predator[i][0] - (16*j))**6+(predator[i][1]-(16*i))**6)
for i in range(predator_swarm):
    predator_best_value[i] = 1 / (0.002 + temp)
...
#Foods
    ...
    for i in range(num_swarm):
        ...
        if (position[i][0] - velocity[i][0] >= 5) or (position[i][0] - velocity[i][0] <= -5):
            velocity[i][0] *= -1
            
        if (position[i][1] - velocity[i][1] >= 5) or (position[i][1] - velocity[i][1] <= -5):
            velocity[i][1] *= -1
...
#Predators
    for i in range(predator_swarm):
        ...
        if (predator[i][0] + predator_velocity[i][0] >= 5) or (predator[i][0] + predator_velocity[i][0] <= -5):
            predator_velocity[i][0] *= -1
        if (predator[i][1] + predator_velocity[i][1] >= 5) or (predator[i][1] + predator_velocity[i][1] <= -5):
